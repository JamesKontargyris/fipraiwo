<table cellspacing="10" cellpadding="10" width="600" bgcolor="#f0f4ff" style="font-family: Arial, Helvetica, sans-serif; font-size:14px;">
    <thead>
        <tr>
            <td valign="top" colspan="2" style="font-size:16px; font-weight:bold; background-color:#00257f; color:white">Explanatory Notes</td>
        </tr>
    </thead>
    <tr>
        <td valign="top" width="200"><strong>Work Order Follows On From</strong></td>
        <td valign="top" width="400">If this work order follows on from, or supersedes, a previous work order, the previous work order reference code is stated here. The previous order can be accessed via the IWO system with this number.</td>
    </tr>
    <tr>
        <td valign="top" width="200"><strong>Lead Unit</strong></td>
        <td valign="top" width="400">For Special Advisers the Lead Unit is always Fipra International.</td>
    </tr>
    <tr>
        <td valign="top" width="200"><strong>Rate and Work Cap</strong></td>
        <td valign="top" width="400">As of July 2016 the 20% inter-Unit discount (IUD) will no longer apply and all fees agreed within an IWO, including success/finders fees, will be paid to the receiving Unit without the application of the IUD. </td>
    </tr>
    <tr>
        <td valign="top" width="200"><strong>Replicon Code</strong></td>
        <td valign="top" width="400">Replicon codes are Fipra International's internal timesheet and billing codes. It is repeated here only for information for Fipra International internal staff.</td>
    </tr>
    <tr>
        <td valign="top" width="200"><strong>Internal Work Order Start Date</strong></td>
        <td valign="top" width="400">The Start date when payments can be made under this IWO.</td>
    </tr>
    <tr>
        <td valign="top" width="200"><strong>Internal Work Order Expiry Date</strong></td>
        <td valign="top" width="400">The date payments will stop under this IWO. After expiry a new IWO must be made if there is more work to be done. The system will send out a reminder email 7 days in advance of the expiry date to informal all parties of the expiry date.</td>
    </tr>
    <tr>
        <td valign="top" width="200"><strong>Green Sheet</strong></td>
        <td valign="top" width="400">Green Sheet is the term for the time-sheet template that Fipra uses and can be obtained by clicking above. Where they are requested, these timesheets need to be submitted within three working days of the end of the month in which the work has been performed. Green Sheets give details of the hours worked and are irrespective of the type of fee structure chosen above. Where they are requested, payment is conditional on greensheets being produced on time and should they be late, this needs to be advised beforehand. If Green Sheets are submitted too late to make the billing cycle it may not be possible to pay the subcontracting Unit or Special Adviser.</td>
    </tr>
</table>