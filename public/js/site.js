...
//  Autocomplete functionality
    ...
    $(".unit_lead_contact_rep_autocomplete").autocomplete({
        ...
        select: function (event, ui) {
            ...
            if (ui.item.additional_info) {
                var newline = String.fromCharCode(13, 10);
                $('textarea[name=' + $(this).data('additional-info-field') + ']').val(ui.item.additional_info.replaceAll('\\n', newline));
            } else {
                $('textarea[name=' + $(this).data('additional-info-field') + ']').val('None');
            }
        }
    });
